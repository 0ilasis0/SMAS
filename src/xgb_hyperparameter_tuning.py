...
def objective(trial: optuna.Trial, X_train: pd.DataFrame, y_train: pd.Series):
    ...
    param = {
        ...
        # 限制 XGBoost 單核心運作，將多核算力交給外層的 Optuna 分配
        'n_jobs': 1,
    ...
    for step, (train_index, val_index) in enumerate(tscv.split(X_train)):
        ...
        # 防禦機制：確保訓練集與驗證集都有兩種標籤 (0 和 1)
        if len(np.unique(y_tr)) < 2 or len(np.unique(y_val)) < 2:
            val_aucs.append(0.5)
            continue

        # 動態計算正負樣本權重，對抗台股多空不平衡
...
def run_optimization(target_total_trials: int):
    ...
    if "date" in df_all.columns:
        ...
        print("⚠️ 找不到明確的 date 欄位，請確認資料時間順序！")

    exclude_cols = [
        FeatureCol.TARGET.value,
        StockCol.DATE.value,
        StockCol.TICKER.value,
        StockCol.CLOSE.value,
        StockCol.VOLUME.value,
        StockCol.OPEN.value,
        StockCol.HIGH.value,
        StockCol.LOW.value,
        StockCol.ADJ_CLOSE.value
    ]
    ...
    print(f"✅ 全域資料合併並【按時間排序】完成！總訓練樣本數: {len(X_train)} 筆")
    print(f"🔍 實際訓練特徵數量: {len(features)} (絕對價格欄位已剔除)")
    ...
    study_name = "XGBoost_Robust_Model_v1"
    ...
    if len(study.trials) > 0 and study.best_trial:
        print(f"🥇 最高風險調整 AUC 分數: {study.best_value:.4f} (在第 {study.best_trial.number} 次尋優找到)")
        print("\n📝 請將以下參數寫入您的 XGBHyperParams：")
