import traceback

import numpy as np
import pandas as pd
from dotenv import dotenv_values
from numpy.typing import NDArray

from const import GlobalCol
from debug import dbg
from path import PathConfig


class FrozenMeta(type):
    def __setattr__(cls, name, value):
        # 攔截對類別屬性的賦值行為
        raise AttributeError(f"無法修改常數屬性: '{name}'")


class MathTool:
    @staticmethod
    def clamp(val: int, min_val: int, max_val: int):
        if max_val < min_val:
            dbg.war("clamp 參數顛倒，已自動交換 min/max")
            min_val, max_val = max_val, min_val
        return max(min(val, max_val), min_val)

class MLTool:
    @staticmethod
    def calculate_scale_weight(y: pd.Series | NDArray, adjustment_factor: float = 1.0) -> float:
        """
        計算正負樣本不平衡的權重比例

        :param y: 目標標籤陣列
        :param adjustment_factor: 調整係數 (預設 1.0)。
                                  若設為 0.5，代表將原始算出的極端權重砍半，
                                  可用來降低神經質模型的「假陽性 (False Positive)」機率。
        """
        pos_count = y.sum()
        neg_count = len(y) - pos_count

        if pos_count == 0: return 1.0

        raw_weight = float(neg_count / pos_count)
        adjusted_weight = max(1.0, raw_weight * adjustment_factor)

        return adjusted_weight

    @staticmethod
    def unscale_probability(p_weighted: np.ndarray, weight: float) -> np.ndarray:
        """
        將經過 scale_pos_weight 膨脹的機率，利用 Log-Odds 數學逆向還原回真實機率分佈。
        這樣能保留樹模型對少數樣本的捕捉力，同時校正輸出尺度。
        """
        # 避免除以 0 的極端情況
        weight = max(weight, 1e-5)
        return p_weighted / (weight * (1.0 - p_weighted) + p_weighted)



class KeyManager:
    """自動從 .env 讀取並管理 API Key 池"""
    @staticmethod
    def _parse_keys_from_env(env_key_name: str, fallback_key_name: str = "") -> list[str]:
        env_path = PathConfig.ENV_FILE
        if not env_path.exists():
            dbg.error(f"❌ 找不到設定檔: {env_path}")
            return []

        try:
            # 徹底擺脫 os.environ 的快取干擾
            env_dict = dotenv_values(dotenv_path=env_path)

            # 雙重保險防呆：同時找尋主名稱與備用名稱
            keys_str = env_dict.get(env_key_name)
            if not keys_str and fallback_key_name:
                keys_str = env_dict.get(fallback_key_name, "")

            if keys_str is None:
                keys_str = ""

            clean_str = str(keys_str).replace('"', '').replace("'", "").replace(" ", "").replace(" ", "").replace("\n", "").replace("\r", "")

            # 切割並過濾掉空字串
            keys_list = [k for k in clean_str.split(",") if k]

            if not keys_list:
                with open(env_path, 'r', encoding='utf-8', errors='ignore') as f:
                    raw_content = f.read(150)
                dbg.error(f"❌ 檔案 {env_path} 存在，但抓不到 {env_key_name} 的 Key！")
                dbg.error(f"👉 以下是系統讀取到的【檔案前 150 個字元】：\n{repr(raw_content)}")
                return []

            return keys_list

        except Exception as e:
            error_details = traceback.format_exc()
            dbg.error(f"❌ 讀取 {env_path} 發生深層崩潰: {e}\n{error_details}")
            return []

    @staticmethod
    def get_gemini_keys() -> list[str]:
        return KeyManager._parse_keys_from_env(GlobalCol.GEMINI_API_KEYS, "GEMINI_API_KEY")

    @staticmethod
    def get_finmind_keys() -> list[str]:
        return KeyManager._parse_keys_from_env(GlobalCol.FINMIND_API_KEYS, "FINMIND_API_KEY")



#
# 狀態總管理
#
class CentralManager:
    def __init__(self):
        self.running: bool = False

central_mg = CentralManager()
