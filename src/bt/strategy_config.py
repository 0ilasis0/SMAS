...
class StrategyConfig:
    """行為樹策略的基礎超參數容器 (此處預設值為 MODERATE 穩健型基底)"""
    stop_loss_tolerance: float = -0.1       # 強制停損容忍度 (-10%)
    trailing_stop_drawdown: float = -0.06   # 移動停損回落容忍度 (-6%)
    ...
    take_profit_target: float = 0.10         # 極端停利目標 (+10%)
    ...
    strong_buy_threshold: float = 0.6        # 強烈買進訊號門檻 (>= 60%)
    strong_buy_capital_ratio: float = 1.0    # 強烈買進時動用的資金比例 (100%)

    conservative_buy_threshold: float = 0.55 # 保守買進訊號門檻 (>= 55%)
    ...
    # ================= 動態水位風控參數 =================
    buy_risk: RiskWeights = field(default_factory=lambda: RiskWeights(heavy=0.2, light=0.1))
