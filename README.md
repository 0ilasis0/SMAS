...
**IDSS** 是一套專為台股與美股設計的**機構級多腦融合量化交易系統**。本專案屏棄了單一指標的盲點，首創「四腦協同運作」架構，完美融合 XGBoost、深度學習 (CNN-LSTM) 與巨觀環境分析 (Market Brain)，並透過 Meta-Learner 進行決策收斂。系統內建嚴密的資產風控模組與 LLM (Gemini) 財報生成，提供從回測到即時推論的一站式完整體驗。
...
### 5. 🤖 Gemini LLM 總裁財報 (Sentiment Oracle)
...
| 模型架構與參數設定 | 預測準度 (DL_AUC) | 平均實財報酬率 (Return) | 風險報酬比 (Sharpe) | 實驗評價 |
| :--- | :--- | :--- | :--- | :--- |
...
### 📉 四張關鍵圖表 (Four-Layer Quant Dashboard)

在回測或歷史推演完成後，系統會產出四層專業的視覺化量化儀表板，讓您徹底看透 AI 的決策邏輯與避險時機：

1. **資金累積曲線與回撤圖 (Equity Curve & Drawdown)：** 展示帳戶總資金隨時間增長的軌跡（藍線），以及下方的最大回撤幅度（紅色區塊）。完美的曲線應「穩定向右上角攀升，無太深的向下紅色凹洞」。
2. **K 線與買賣點標示圖 (Trade Markers)：** 標示出 AI 決定 🟢 買進與 🔴 賣出的精確位置。方便肉眼直接驗證 AI 是否做到「買在起漲點，賣在反轉前」。
3. **個股 AI 勝率與真實走勢圖 (AI Probability & Stock Price)：** 專注於個股本身的決策心電圖：
   * **🟠 橘色實線 (綜合決策勝率)：** 該股最終上漲機率。突破 60% 區間通常為強烈買進訊號。
   * **⚪ 灰色實線 (個股股價)：** 疊加真實走勢，方便對照勝率高低點與股價轉折的關係。
   * **🟥 紅色警戒底塊 (大盤危險區)：** 與第四層同步連動，標示出系統性風險極高、強制禁止買進的區間。
4. **大盤防禦雷達與指數對照圖 (Market Brain & TWII Index)：** 系統的「宏觀保命符」，專注監控系統性崩盤：
   * **🟣 紫色實線 (大盤環境安全度)：** 結合 VIX、外資期貨空單與大盤技術面算出的安全機率。
   * **🔴 紅色水平虛線 (動態防禦門檻)：** 系統透過 F-Beta Score 與 PR 曲線尋優算出的「最佳動態防禦切點」。一旦紫色實線跌破此紅線，即觸發全系統的紅色警戒底塊。
   * **⚪ 淺灰實線 (大盤加權指數 ^TWII)：** 疊加真實大盤走勢，用於直觀驗證防禦雷達是否精準在「大盤崩盤前夕」拉響警報。
...
當您點擊「預測今日走勢」時，系統會吐出四顆大腦的決策分數，並附帶底層模型的健康度診斷：
...
* **5. 新聞情緒分數 (Gemini Sentiment): `[例如：3 / 10]`**
  ...
  * **4~6 分：** 消息面平淡，無明顯利多利空。
  * **7~10 分：** 釋出大利多或市場極度樂觀。
* **6. ⚙️ AI 引擎底層健康度 (白盒化進階資訊):**
  * 系統內建模型診斷面板，自動擷取各模型在訓練階段的 **全局盲測準確度 (Global OOF AUC)**，並將複雜的數學指標翻譯為直觀的紅綠燈號（🟢 極佳 / 🟡 正常 / 🔴 重新適應中），確保您隨時掌握神經網路的健康狀態。面板同時會揭露大盤雷達的「實戰動態防禦警戒線」，讓 AI 的底層風控邏輯完全透明化。

> 💡 **總結來說：** 一個完美的 **「強力買進 (Strong Buy)」** 訊號，通常會伴隨著：`XGB 與 DL 雙雙突破 60%` + `大盤安全度 > 50%` + `新聞情緒 >= 6` + `AI 健康度亮綠燈`，最後由 Meta 總指揮拍板定案！
...
> 💡 **首次使用提示：**
> 進入系統後，請先至側邊欄輸入欲追蹤的股票代號。進入「全系統重置與升級」分頁點擊 **「所有模型重新訓練」** 完成四大 AI 模型訓練，即可開始量化推論。



## ⚠️ 免責聲明 (Disclaimer)

本專案 (`IDSS-Quant`) 僅供學術研究、量化模型開發與程式設計交流之用。系統產出的任何預測勝率、買賣訊號、建議股數、智慧定價或財報內容，**皆不構成任何形式的投資建議**。金融市場具備高度不確定性，使用者應自行評估風險，並對實際交易之盈虧負完全責任。